...
def smoothen_and_set_paths(self):
    ...
    for i in range(self.weft_count):
        for j in range(self.warp_count):
            if j != 0:
                weft_segment = __generate_smooth_weft_segment(self, i,j)
    ...
    for j in range(self.warp_count):
        i = 1 # skip first
        skip_prev= False
        while i < self.weft_count:
            warp_segment = []
            if not skip_prev: 
                warp_segment = __generate_smooth_warp_segment(self, i,i-1, j)
            else:
                warp_segment = __generate_smooth_warp_segment(self, i,i-2, j)
...
def __generate_smooth_warp_segment(self, i, prevI,j):
    ...
    prev_weft_center, curr_weft_center = self.weft_paths_centers[prevI][j], self.weft_paths_centers[i][j]
    ...
    if i != self.weft_count-1:
        next_pattern, next_weft_center = self.pattern[i+1][j], self.weft_paths_centers[i+1][j]
...
def __generate_smooth_weft_segment(self, i,j):
    ...
    if prev_pattern == curr_pattern:
        raise ValueError("float not suppored")
