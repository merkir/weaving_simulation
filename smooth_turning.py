...
def smoothen_and_set_paths(self):
    weft_paths = [[] for _ in range(self.fabricYarnSimulations.weft_count)]
    ...
    for i in range(self.fabricYarnSimulations.weft_count):
        ...
        weft_paths[i] = __add_extra_start_and_end(self, weft_paths[i], self.fabricYarnSimulations.weft_yarns[i].yarn_diameter, "weft")
        # self.weft_yarns[i].set_yarn_path(self.weft_paths[i])


    warp_paths = [[] for _ in range(self.fabricYarnSimulations.warp_count)]
    for j in range(self.fabricYarnSimulations.warp_count):
        ...
        warp_paths[j] = __add_extra_start_and_end(self, warp_paths[j], self.fabricYarnSimulations.warp_yarns[j].yarn_diameter, "warp")
...
def __smoothen_line_segment(self, start, end):
        ...
        smooth_segment = []
        for i in range(1, self.fabricProperties.control_points_per_turning):
            alph = i / self.fabricProperties.control_points_per_turning 
...
def __generate_smooth_warp_segment(self, iIdx, previIdx, j):
    ...
    print(f"__generate_smooth_warp_segment: iIdx={iIdx}, previIdx={previIdx}, prevI={prevI}, i={i}, len(self.weft_paths_centers[j]): {len(self.weft_paths_centers[j])}")

    """geneate weft segment form the pervious intersection (i,j-1) to the current one (i-j)"""
    prev_pattern, curr_pattern =  self.fabricYarnSimulations.pattern[prevI][j], self.fabricYarnSimulations.pattern[i][j]
    ...
    if i < len(self.fabricYarnSimulations.pattern) -1 and iIdx < len(self.weft_paths_centers[j]) - 1 :
        next_pattern, next_weft_center = self.fabricYarnSimulations.pattern[i+1][j], self.weft_paths_centers[j][iIdx+1]
    ...
    warp_radius = self.fabricYarnSimulations.warp_yarns[i].yarn_diameter/2
    prev_weft_radius, current_weft_radius = self.fabricYarnSimulations.weft_yarns[prevI].yarn_diameter/2, self.fabricYarnSimulations.weft_yarns[i].yarn_diameter/2
    
    rA, rB = (prev_weft_radius + warp_radius)/self.fabricProperties.compress_factor , (current_weft_radius + warp_radius)/self.fabricProperties.compress_factor 
    forward = curr_weft_center - prev_weft_center
    forward_norm = np.linalg.norm(forward/self.fabricProperties.compress_factor )
    ...
    segment = []
    offset_angle = 0
    compressed_circular_angle_slice = compressed_circular_angle / self.fabricProperties.control_points_per_turning

    if prev_pattern == 0:
        # the half on previous warp
        for i in range(self.fabricProperties.control_points_per_turning):
        ...
        for i in range(self.fabricProperties.control_points_per_turning):
            ...
            if i == 0:
                ...
                for i in range(1, self.fabricProperties.control_points_per_turning * (i-prevI)):
                    alph = i / (self.fabricProperties.control_points_per_turning * (i-prevI))
...
def __generate_smooth_weft_segment(self, i,jIdx):
    ...
    """geneate weft segment form the pervious intersection (i,j-1) to the current one (i-j)"""
    prev_pattern, curr_pattern =  self.fabricYarnSimulations.pattern[i][prevJ], self.fabricYarnSimulations.pattern[i][j]
    ...
    weft_radius = self.fabricYarnSimulations.weft_yarns[i].yarn_diameter/2
    prev_warp_radius, current_warp_radius = self.fabricYarnSimulations.warp_yarns[prevJ].yarn_diameter/2, self.fabricYarnSimulations.warp_yarns[j].yarn_diameter/2
    ...
    compressed_circular_angle = circular_angle / self.fabricProperties.compress_factor 
